...
def build_tts_manifest(
    text: str, language: str, target_emotion: str, tts_dir: Path, file_prefix: str = ""
) -> TTSManifest:
    text_hash = sha256(text.encode()).hexdigest()[:5]
    filename = (
        f"{file_prefix + '.' if file_prefix else ''}{target_emotion}.{text_hash}.wav"
    )
...
def build_vc_manifest(
    ...
    filename = f"{file_prefix + '_' if file_prefix else ''}from_{source_audio_path.stem}_to_{voice_reference_path.stem}.wav"
...
def build_vg_manifest(
    audio_path: Path, face_reference_path: Path, output_path: Path, target_emotion: str | None = None
) -> VGManifest:
    return VGManifest(
